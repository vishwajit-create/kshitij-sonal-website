// ── Guard ──────────────────────────────────────────────────
fetch('/api/admin/check').then(r=>r.json()).then(d=>{ if(!d.isAdmin) window.location='/admin/login.html'; });
...
// Handle publish form submissions
...
async function openEdit(type, id) {
  ...
  const fields = buildEditFields(type, item);
...
function buildEditFields(type, item) {
  const g = (label, name, val, tag='input', extra='') =>
    `<div class="form-group"><label class="form-label">${label}</label>${tag==='textarea'?`<textarea name="${name}" class="form-textarea" style="min-height:120px">${val||''}</textarea>`:`<input type="${extra||'text'}" name="${name}" class="form-input" value="${(val||'').replace(/"/g,'&quot;')}">`}</div>`;
...
async function loadCustomize() {
  try {
    ...
    // Hero
    const heroFields = ['siteTitle','tagline','kicker','heading','subtitle','metaLine','cta1Text','cta1Link','cta2Text','cta2Link','footerText','stat1Value','stat1Label','stat2Value','stat2Label','stat3Value','stat3Label','stat4Value','stat4Label'];
    heroFields.forEach(f => { const el=$('c-'+f); if(el) el.value = h[f]||''; });
    // About
