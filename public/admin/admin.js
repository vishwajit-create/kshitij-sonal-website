...
function switchPanel(name) {
  ...
  $('topbar-title').textContent = { dashboard: 'Dashboard', publish: 'Publish Content', manage: 'Manage Content', customize: 'Customize', settings: 'Settings', enquiries: 'Enquiries' }[name] || name;
...
async function loadDashboard() {
  try {
    ...
    if($('s-enquiries')) $('s-enquiries').textContent = stats.enquiries||0;
    const badge=$('enq-badge');if(badge){badge.textContent=stats.unread||0;badge.style.display=stats.unread?'inline':'none';}
...
async function loadEnquiries() {
  const el = $('enq-list');
  el.innerHTML = '<div style="color:var(--muted);padding:2rem;text-align:center">Loading...</div>';
  try {
    const list = await api('/api/admin/enquiries');
    const badge = $('enq-badge');
    if (!list.length) {
      el.innerHTML = '<div style="color:var(--muted);padding:2rem;text-align:center">No enquiries yet.</div>';
    ...
    el.innerHTML = list.map(e=>`
      <div class="settings-card" id="enq-${e.id}" style="${e.read?'':'border-left:3px solid var(--gold)'}">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:0.5rem">
          <div>
            <strong style="color:var(--text)">${e.name}</strong>
            <span style="color:var(--muted);font-size:0.8rem;margin-left:0.5rem">&lt;${e.email}&gt;</span>
            ${!e.read?'<span style="background:var(--gold);color:#000;border-radius:4px;padding:1px 6px;font-size:0.7rem;margin-left:0.5rem">NEW</span>':''}
          </div>
          <div style="display:flex;gap:0.5rem">
            ${!e.read?`<button class="btn btn-ghost btn-sm" onclick="markRead('${e.id}')">✓ Mark Read</button>`:''}
            <button class="btn btn-danger btn-sm" onclick="deleteEnquiry('${e.id}')">🗑</button>
          </div>
        </div>
        <div style="color:var(--text-sec);font-size:0.85rem;margin-top:0.4rem"><strong>Subject:</strong> ${e.subject}</div>
        <div style="color:var(--text-sec);font-size:0.85rem;margin-top:0.25rem"><strong>Date:</strong> ${fmtDate(e.date)}</div>
        <div style="margin-top:0.75rem;padding:0.75rem;background:var(--bg);border-radius:8px;color:var(--text-sec);font-size:0.875rem;white-space:pre-wrap">${e.message}</div>
      </div>`).join('');
  } catch(err) { el.innerHTML = '<div style="color:var(--muted);padding:2rem">Failed to load enquiries.</div>'; }
}
async function markRead(id) {
  await api('/api/admin/enquiries/'+id+'/read', {method:'PATCH'});
  loadEnquiries();
}
async function deleteEnquiry(id) {
  if(!confirm('Delete this enquiry?')) return;
  const r = await api('/api/admin/enquiries/'+id, {method:'DELETE'});
  if(r.success) { const el=$('enq-'+id); if(el) el.remove(); }
  loadEnquiries();
}
